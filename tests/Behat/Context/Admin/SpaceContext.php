...
use App\Context\Admin\Space\Application\Gateway\FindSpaceById;
use App\Context\Admin\Space\Application\Gateway\UpdateSpace;
use Behat\Behat\Context\Context;
use Behat\Gherkin\Node\TableNode;
use App\Context\Admin\Space\Application\Gateway\BrowseSpaces as BaseBrowseSpaces;
use App\Context\Admin\Space\Application\Gateway\CreateSpace as BaseCreateSpace;
use App\Context\Admin\Space\Application\Gateway\FindSpaceByCode as BaseFindSpaceByCode;
use App\Context\Admin\Space\Application\Gateway\FindSpaceById as BaseFindSpaceById;
use App\Context\Admin\Space\Application\Gateway\CloseSpace;
use App\Context\Admin\Space\Application\Gateway\PublishSpace;
...
final class SpaceContext implements Context
{
    ...
    public function iCreateThisSpace()
    {
        ...
        Assert::allIsInstanceOf($this->responses, BaseCreateSpace\Response::class);
    }
    ...
    public function iShouldBeAbleToFindMySpaceWithHisIdentifier()
    {
        foreach ($this->responses as $response) {
            $result = ($this->findSpaceByIdGateway)(BaseFindSpaceById\Request::fromData($response->data()));
    ...
    {
        foreach ($this->responses as $response) {
            $result = ($this->findSpaceByCodeGateway)(BaseFindSpaceByCode\Request::fromData([
                'code' => $code,
            ]));
    ...
    public function iAlreadyHaveASpaceWithCode(TableNode $table)
    {
        ...
        /** @var array $row */
        foreach ($table as $row) {
            $this->responses[] = ($this->findSpaceByCodeGateway)(BaseFindSpaceByCode\Request::fromData($row));
        }
    ...
    {
        $this->responses = ($this->findSpacesGateway)(BaseBrowseSpaces\Request::fromData())->data();
    ...
    public function iShouldHaveAtLeastOneSpaceWithCode(TableNode $table)
    {
        ...
        foreach ($this->responses as $response) {
            $response = ($this->findSpaceByIdGateway)(BaseFindSpaceById\Request::fromData($response));
    ...
    {
        foreach ($table as $row) {
            $response = ($this->findSpaceByCodeGateway)(BaseFindSpaceByCode\Request::fromData($row));
    ...
    {
        $space = ($this->findSpaceByCodeGateway)(BaseFindSpaceByCode\Request::fromData(
    ...
    {
        try {
            ($this->findSpaceByCodeGateway)(BaseFindSpaceByCode\Request::fromData(
                $this->responses[0]->data()
            ));
