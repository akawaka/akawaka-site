...
final class Instrumentation
{
    ...
    {
        $this->logger->info('category.remove', $request->data());
    }

    public function success(Response $response): void
    {
        $this->logger->info('category.remove_success', $response->data());
    }

    public function error(Request $request, string $reason): void
    {
        $this->logger->error('category.remove_error', array_merge(
            $request->data(),
            [' reason' => $reason]
        ));
    }
