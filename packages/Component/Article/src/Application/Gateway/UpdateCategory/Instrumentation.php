...
final class Instrumentation
{
    ...
    {
        $this->logger->info('category.update', $request->data());
    }

    public function success(Response $response): void
    {
        $this->logger->info('category.update_success', $response->data());
    }

    public function error(Request $request, string $reason): void
    {
        $this->logger->error('category.update_error', array_merge(
            $request->data(),
            [' reason' => $reason]
        ));
    }
