...
final class Instrumentation
{
    ...
    {
        $this->logger->info('article.publish', $request->data());
    }

    public function success(Response $response): void
    {
        $this->logger->info('article.publish_success', $response->data());
    }

    public function error(Request $request, string $reason): void
    {
        $this->logger->error('article.publish_error', array_merge(
            $request->data(),
            [' reason' => $reason]
        ));
    }
