...
final class Instrumentation
{
    ...
    {
        $this->logger->info('category.find_by_id', $request->data());
    }

    public function success(Response $response): void
    {
        $this->logger->info('category.find_by_id_success', $response->data());
    }

    public function error(Request $request, string $reason): void
    {
        $this->logger->error('category.find_by_id_error', array_merge(
            $request->data(),
            [' reason' => $reason]
        ));
    }
