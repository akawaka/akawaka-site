...
final class Instrumentation
{
    ...
    {
        $this->logger->info('article.find_by_code', $request->data());
    }

    public function success(Response $response): void
    {
        $this->logger->info('article.find_by_code_success', $response->data());
    }

    public function error(Request $request, string $reason): void
    {
        $this->logger->error('article.find_by_code_error', array_merge(
            $request->data(),
            [' reason' => $reason]
        ));
    }
